function init_sim_emitter_signal(sat::Satellite, gnss_system::GNSSSystem, sample_freq, interm_freq)
    ...
    samples -> _sim_sat_signal(samples, gnss_system, init_code_phase, init_carrier_phase, init_doppler, sample_freq, init_amplitude, sat.svid)
end

function _sim_sat_signal(samples, gnss_system, code_phase, carrier_phase, doppler, sample_freq, amplitude, svid)
    ...
    sampled_code = gnss_system.calc_sampled_code(1:samples, code_freq_with_doppler, code_phase, sample_freq, svid)
