...
function init_sim_emitter_signal(jammer::CWJammer, gnss_system::S, sample_freq, interm_freq) where S <: AbstractGNSSSystem
    ...
    num_samples -> _sim_cw_jammer_signal(num_samples, gnss_system, init_carrier_phase, init_doppler, sample_freq, init_amplitude, interm_freq), EmitterInternalStates(init_doppler, init_carrier_phase, NaN)
end
