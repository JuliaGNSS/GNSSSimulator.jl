@testset "Satellite" begin
    ...
    @testset "Satellite signal" begin
        ...
        @test @inferred(GNSSSimulator.get_code_phase(next_sat)) ≈ 100 + 1000Hz / 1540 / 2e6Hz
        ...
        @test @inferred(GNSSSimulator.get_system(next_sat)) == gpsl1

        get_steer_vec(doa, attitude) = 1 # No steering vector

        signal = @inferred get_signal(next_sat, nothing, get_steer_vec)
