@testset "Attitude" begin
    @testset "Static Attitude" begin
        ...
        next_noisy_static_attitude = @inferred propagate(noisy_static_attitude, 1ms)
    ...
    @testset "Dynamic Attitude" begin
        ...
        next_dynamic_attitude = @inferred propagate(dynamic_attitude, 1s)
        forward_dynamic_attitude = @inferred propagate(dynamic_attitude, 20s)
        linear_dynamic_attitude = @inferred LinearDynamicAttitude(RotXYZ(0.0, 0.1, 0.2), 0.1rad/s, 0.1rad/s, 0.1rad/s)
        next_linear_dynamic_attitude = @inferred propagate(linear_dynamic_attitude, 1s)
        ...
        next_noisy_dynamic_attitude = @inferred propagate(noisy_dynamic_attitude, 1s)
        @test next_noisy_dynamic_attitude.attitudes == attitudes2
        noisy_linear_dynamic_attitude = @inferred NoisyLinearDynamicAttitude(RotXYZ(0.0, 0.1, 0.2), 0.0rad/s, 0.0rad/s, 0.0rad/s, 0.1, 0.2, 0.3)
        next_noisy_linear_dynamic_attitude = @inferred propagate(noisy_linear_dynamic_attitude, 1s)
